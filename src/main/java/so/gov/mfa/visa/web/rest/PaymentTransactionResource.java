...
/**
 * REST controller for managing {@link so.gov.mfa.visa.domain.PaymentTransaction}.
 */
@RestController
@RequestMapping("/api")
public class PaymentTransactionResource {
    ...
    /**
     * {@code SEARCH  /_search/payment-transactions?query=:query} : search for the paymentTransaction corresponding
     * to the query.
     *
    ...
    public ResponseEntity<List<PaymentTransactionDTO>> searchPaymentTransactions(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of PaymentTransactions for query {}", query);
        Page<PaymentTransactionDTO> page = paymentTransactionService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
        }
}
